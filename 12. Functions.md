...
프로그래밍에서 함수는 입력(input)을 받아 출력(output)을 내보내는 일련의 과정을 정의한 것으로 하나의 실행 단위이다.
...
함수에 대해 `typeof` 연산자를 사용하면 `function`이 반환된어 오해할 수 있으나, `function`은 top-level 빌트인 타입이 아니라 다만 `object`의 subtype 중 하나이다. 즉, 자바스크립트의 함수는 객체이다.

- 자바스크립트의 함수는 다른 객체들과 달리 호출할 수 있는 객체(callable object)이다. 이는 내부 메서드로 `[[Call]]`을 가지기 때문이다.
...
식별자 `foo`를 선언하여 확보한 메모리 공간은 함수 리터럴이 평가되어 저장된 메모리 공간을 참조하게 된다. 함수 리터럴로 함수를 정의하는 방식은 [12.3 함수 정의하기 - 함수 표현식](#함수-표현식function-표현식)을 참고한다.
