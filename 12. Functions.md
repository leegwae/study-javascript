...
함수는 다른 객체들과 달리 호출할 수 있는 객체(callable object)이다. 호출할 수 있는 객체는 다른 객체들과 달리 내부 메서드 `[[Call]]` 등을 가진다. 내부 메서드에 대해서는 [Property Attributes](https://github.com/leegwae/study-javascript/blob/main/Property%20Attributes.md)를 참고한다.
...
- 매개변수는 함수 본문에 지역적인 것으로 취급되어. 함수가 호출될 때 지역 변수처럼 생성된다. 따라서 전달된 인자가 없으면 재할당이 일어나지 않으므로 `undefined`로 접근된다.
