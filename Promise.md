...
우리가 원하는 값은 비동기 처리가 완료되었을 때 프로미스가 가진 결과 값이다. 프로미스는 프로미스가 가진 결과 값을 처리할 수 있는 콜백을 인자로 전달받는 프로토타입 메서드를 제공한다. 각 프로토타입 메서드는 프로미스 객체를 반환하므로, 프로미스 체이닝(promise chaining)을 사용할 수 있다.
