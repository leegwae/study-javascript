...
변수는 초기화 단계에서 확보한 메모리 공간에 `undefined` 값을 갖는다. 따라서 변수에 값을 할당하지 않는다면 변수 선언문 이후 접근한 변수는 `undefined` 값을 반환한다.
...
그런데 변수 선언문 이전에 접근한 변수는 `var` 키워드로 선언했다면 `undefined`를 반환하지만 `let`/`const`로 선언했다면 `ReferneceError`를 발생시킨다.

```javascript
console.log(a);		// undefined
var a;
console.log(b);		// RefenreceError
let b;
```

 이는 전자의 경우 초기화 단계가 런타임 이전에, 후자의 경우 런타임에 실행되기 때문이다. [변수 선언의 실행 시점](#변수-선언의-실행-시점)과 [변수 호이스팅](#변수-호이스팅)을 참고한다.
...
2. 할당문(`a = 1;`) 실행(런타임): `a`에 `1`이 할당되었다.
...
변수의 범위(스코프; scope)는 함수를 기준으로 나뉜다.
...
>
>1. **소스코드 평가 과정**: 모든 선언문(변수 선언문, 함수 선언문 등)을 찾아내어 실행한다.
>2. **소스코드 실행 과정**: 모든 선언문을 제외한 소스코드를 한줄씩 실행한다.
...
**자바스크립트에서 선언은 런타임(runtime) 이전에 실행된다**. 위 코드의 경우 변수 `a`에 대한 선언 단계가 런타임 이전에 이루어졌으므로 런타임 이후 `console.log(a)`를 실행해도 `ReferenceError`가 발생하지 않는다(하지만 `var`로 선언했을 경우이며, `let`이나 `const`로 선언했을 경우 `RefenreceError`가 발생한다). *선언이 상단으로 끌어올려졌다*고 하며, 이를 **호이스팅(hositing)**이라고 한다.



## 변수 호이스팅

**변수 호이스팅(variable hoisting)**은 변수 선언이 상단으로 끌어올려진 것을 말한다.
