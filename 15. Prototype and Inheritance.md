...
- **프로토타입**(prototype)은 상위 객체로서 다른 객체에 공유 프로퍼티를 제공한다. 즉, 프로퍼티를 상속한다. 프로퍼티를 상속받은 객체는 하위 객체로서 상위 객체의 프로퍼티를 사용할 수 있다.
...
객체의 프로토타입은 객체를 생성하는 방식에 따라 달라진다.



### 리터럴로 생성한 프로토타입

- 리터럴 표기법으로 생성한 객체와 생성자 함수로 생성한 객체는 세부 내용이 다를 수 있다. 
...
#### `Function` 생성자 함수와 함수 리터럴

- `Function` 생성자 함수로 생성한 함수와 함수 선언문, 함수 표현식을 평가하여 생성한 함수는 세부 내용(스코프의 생성, 클로저)이 다르다.
