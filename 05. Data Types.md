...
- ECMAScript의 데이터 타입은 **원시(primitive)**와 **객체(object)**로 나누어볼 수 있다.
...
- ECMAScript2020 기준 원시 데이터 타입에는 7가지 타입이 있다.
  - Undefined Type
  - Null Type
  - Boolean Type
  - String Type
  - Number Type
  - Symbol Type(ECMAScript2015에 도입)
  - BigInt Type(ECMAScript2020에 도입)
...
따라서 변수에 값이 없다는 것을 표현하려면 `undefined`보다 `null`을 할당하는 것이 적절한다.
...
- `number` 타입은 64비트 부동소수점 형식을 따르기 때문에 모든 수를 실수로 처리한다.
...
- 템플릿 리터럴은 새로운 문자열 표기법으로 ES6부터 도입되었다.
...
원시 값은 변경 불가능(immutable)한 값이고 객체는 변경 가능(mutable)한 값이다. 이것은 변수에 원시 값이 할당되었을 때 변수 값을 변경 불가능하다는 뜻이 아니다. 변수는 `const`로 선언되지 않았다면 그 값을 교체할 수 있다.
...
그러나 참조된 객체의 프로퍼티를 수정하면 원본 변수의 값도 수정되는 것처럼 보인다. [값에 의한 전달(pass by value)]((https://github.com/leegwae/study-javascript/blob/main/03.%20Variable%20and%20Constant.md/#34-pass-by-value))을 참고한다.
