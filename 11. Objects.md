...
함수 선언문, 함수 표현식, 클래스로 정의된 함수는 `new` 연산자와 함께 사용하면 객체의 인스턴스를 생성하는 **생성자 함수**로서 동작한다. 특별히 객체의 타입을 정의하는 함수를 **생성자(constructor)**라고 한다. 생성자 함수는 자바스크립트가 제공하는 표준 내장 객체를 생성하는 **빌트인(built-in) 생성자 함수**이거나 사용자가 정의한 타입의 객체를 생성하는 **사용자 정의(user-defined) 생성자 함수**이다.
...
- 생성자 함수 객체의 `prototype` 프로퍼티를 사용하여 프로퍼티를 동적으로 생성한다.
...
- `Cat.prototype`을 상속하는 `cat1` 객체에 프로퍼티를 동적으로 생성한다.
- 이 경우 `cat1` 객체에만 `color` 프로퍼티가 동적으로 생성되며 `Cat.prototype`을 상속하는 모든 객체에 `color` 프로퍼티가 동적으로 생성되지 않는다. 따라서 `cat2.color`는 `undefined`를 반환한다.
...
- **전역 객체**(global object)는 표준 내장 객체(`String`, `Number` 등)와 런타임 환경에 따른 호스트 객체(브라우저라면 클라이언트 사이트 웹 API, Node라면 호스트 API), `var`로 선언한 전역 변수와 암묵적 전역, 전역 함수를 프로퍼티로 가진다.

- 브라우저 환경에서 전역 객체를 가리키는 식별자는 **`window`**이다. 전역 변수는 `window.변수이름` 구문을 통해 변경하고 접근할 수 있다. 이떄 `window`를 생략할 수 있다.
